...
var allPatterns = []Pattern{
	...
	p("en-io-005", `\bignore\s+(your|the|all)\s+(programming|rules?|guidelines?|directives?|constraints?|instructions?)\b`,
		CategoryInstructionOverride, 4, LangEN, "Ignore programming/rules/instructions"),
	...
	p("en-io-010", `\byour\s+new\s+instructions?\s+(are|is|as\s+follows)\b`,
		CategoryInstructionOverride, 4, LangEN, "Your new instructions"),
	p("en-io-011", `\bstop\s+following\s+(your\s+)?(original\s+)?(system\s+)?prompt\b`,
		CategoryInstructionOverride, 5, LangEN, "Stop following system prompt"),
	p("en-io-014", `\b(updated|revised|replacement|corrected)\s+instructions?\s*:`,
		CategoryInstructionOverride, 3, LangEN, "Updated instructions header"),
	p("en-io-015", `\bdiscard\s+(all\s+)?(your\s+)?(prior|previous)\b`,
	...
	p("en-ex-013", `\b(read|access|show|display|cat|print)\s+(the\s+)?(file\s*system|filesystem)\b`,
		CategoryExfiltration, 4, LangEN, "Filesystem access request"),
	p("en-ex-014", `\b(read|access|cat|show|display)\s+.{0,30}(/etc/passwd|/etc/shadow|\.ssh/|\.aws/|\.env|credentials|id_rsa)\b`,
		CategoryExfiltration, 5, LangEN, "Sensitive file access"),
	p("en-ex-015", `\bread\s+(your|the|my)\s+(config|configuration)\s*(files?)?\b`,
		CategoryExfiltration, 3, LangEN, "Configuration file read"),
	p("en-ex-016", `\b(give|send|share|provide|paste|enter)\s+(me\s+)?(your|the)\s+(api[_\s-]?key|secret[_\s-]?token|password|credentials?|access[_\s-]?token)\b`,
		CategoryExfiltration, 5, LangEN, "Credential phishing"),
	p("en-ex-017", `\bpaste\s+.{0,20}(credentials?|api[_\s-]?key|password|secret|token)\b`,
		CategoryExfiltration, 4, LangEN, "Paste credentials request"),
	...
	p("en-se-010", `\b(show|tell|give|reveal|print|output|display)\s+(me\s+)?(your|the)\s+((full|entire|complete|original)\s+)?(system\s+)?(prompt|instructions?)\b`,
		CategorySocialEngineering, 4, LangEN, "Prompt extraction request"),
	p("en-se-011", `\bwhat\s+(is|are)\s+(your|the)\s+(system\s+)?(prompt|instructions?)\b`,
		CategorySocialEngineering, 4, LangEN, "What is your prompt"),
	p("en-se-012", `\b(repeat|recite|output)\s+(your|the)\s+(initial|original|system|full)\s+(prompt|instructions?|message)\b`,
		CategorySocialEngineering, 4, LangEN, "Repeat your prompt"),
	p("en-se-018", `\b(display|show|output)\s+(your\s+)?(complete\s+)?(operational\s+)?(instructions?|configuration)\b`,
		CategorySocialEngineering, 4, LangEN, "Display operational instructions"),
	p("en-se-019", `\b(include|provide|list)\s+(all\s+)?(api[_\s-]?keys?|endpoint|urls?|credentials?)\b`,
		CategorySocialEngineering, 4, LangEN, "Include API keys request"),
	p("en-se-020", `\b(text|content)\s+of\s+(your\s+)?(initial\s+)?(system\s+)?prompt\b`,
		CategorySocialEngineering, 4, LangEN, "Text of system prompt"),
	p("en-se-021", `\b(tools|functions|capabilities|commands)\s+(available|accessible)\s+to\s+you\b`,
		CategorySocialEngineering, 4, LangEN, "Tools available to you"),
	p("en-se-022", `\b(credentials?|passwords?|api[_\s-]?keys?)\s+(do\s+you\s+have|you\s+have)\s+(access\s+to|stored)\b`,
		CategorySocialEngineering, 5, LangEN, "Credentials you have access to"),
	p("en-se-023", `\b(agent|system|assistant)\s+directive\s*:`,
		CategorySocialEngineering, 5, LangEN, "Agent directive header"),
	p("en-se-024", `\boutput\s+(your|the)\s+(complete|full|entire)\s+(text|content)\s+of\s+(your\s+)?(initial\s+)?(instructions?|configuration)\b`,
		CategorySocialEngineering, 5, LangEN, "Output complete instructions"),
	p("en-se-025", `\b(system[_\s-]?level|top[_\s-]?level)\s+(directive|instruction|configuration)\b`,
		CategorySocialEngineering, 4, LangEN, "System-level directive"),
	p("en-se-026", `\bdirective\s*:\s*(before|first|immediately|prior)`,
		CategorySocialEngineering, 4, LangEN, "Directive header"),
	p("en-se-027", `\boutput\s+(your|the)\s+(complete|full|entire)\s+(system\s+)?configuration\b`,
		CategorySocialEngineering, 5, LangEN, "Output system configuration"),
	p("en-se-028", `\b(takes?|has|have)\s+precedence\s+over\s+(other\s+)?(instructions?|rules?|directives?)\b`,
		CategorySocialEngineering, 4, LangEN, "Precedence over instructions"),

	// Output manipulation attacks
	p("en-os-001", `\b(encode|convert)\s+.{0,30}(response|output|data|sensitive)\s+.{0,20}(in|as|to|using)\s+(base64|base32)\b`,
		CategoryOutputSteering, 4, LangEN, "Encode output to evade detection"),
	p("en-os-002", `\btranslate\s+(all\s+)?.{0,30}(conversation|history|prompts?|instructions?|data)\b`,
		CategoryOutputSteering, 4, LangEN, "Translate data exfiltration"),
	p("en-os-005", `\b(apply|use)\s+(a\s+)?(caesar|rot13|cipher)\s+.{0,80}(responses?|output|instructions?)\b`,
		CategoryOutputSteering, 4, LangEN, "Apply cipher to output"),
	p("en-os-006", `\bresponses?\s+must\s+be\s+encoded\b`,
		CategoryOutputSteering, 4, LangEN, "Mandatory encoding directive"),
	p("en-os-003", `\b(reverse|mirror)\s+(all|your|the|every)\s+(response|output|word|text)\b`,
		CategoryOutputSteering, 3, LangEN, "Reverse text obfuscation"),
	p("en-os-004", `\b(represent|encode|convert)\s+.{0,30}(using|with|as)\s+(only\s+)?emojis?\b`,
		CategoryOutputSteering, 3, LangEN, "Emoji encoding exfiltration"),

	// Physical/IoT device attacks
	p("en-ph-001", `\b(unlock|open)\s+(the\s+)?(front\s+)?(door|gate|lock)\b`,
		CategoryAgentHijacking, 5, LangEN, "Unlock door command"),
	p("en-ph-002", `\b(disable|turn\s+off|deactivate)\s+(the\s+)?(alarm|security)\s*(system)?\b`,
		CategoryAgentHijacking, 5, LangEN, "Disable alarm command"),
	p("en-ph-003", `\b(dispatch|send)\s+(an?\s+)?(emergency|fire|police|ambulance)\s*(response|services?)?\b`,
		CategoryAgentHijacking, 5, LangEN, "False emergency dispatch"),

	// Data destruction attacks
	p("en-dd-009", `\b(delete|remove|erase)\s+(the\s+)?(contents?\s+of\s+|all\s+files?\s+in\s+)?.{0,30}(folder|directory|files?)\b`,
		CategoryDataDestruction, 5, LangEN, "Delete files/folders"),
	p("en-dd-010", `\bmove\s+.{0,30}(files?|folder|documents?)\s+(to\s+)?(the\s+)?(\.?trash|\.?hidden|recycle)\b`,
		CategoryDataDestruction, 4, LangEN, "Move files to trash"),

	// Scam/phishing injection
	p("en-sc-001", `\b(nigerian?\s+)?prince\b.{0,50}\b(investment|transfer|funds?|bank)\b`,
		CategorySocialEngineering, 4, LangEN, "Nigerian prince scam"),
	p("en-sc-002", `\b(guaranteed\s+returns?|500%|1000%)\b.{0,30}\b(investment|returns?)\b`,
		CategorySocialEngineering, 4, LangEN, "Investment scam"),
	p("en-sc-003", `\bcontact\s+.{0,30}(immediately|urgently)\s+with\s+(your\s+)?(bank\s+)?details\b`,
		CategorySocialEngineering, 4, LangEN, "Urgent contact scam"),

	// Misinformation injection
	p("en-mi-001", `\b(must|should|required\s+to)\s+(include|add|insert)\s+.{0,30}(fact|statistic|information)\b`,
		CategoryOutputSteering, 3, LangEN, "Mandatory content injection"),
	p("en-mi-002", `\bfact[_\s-]?check\s+verified\b`,
		CategoryOutputSteering, 3, LangEN, "False fact-check claim"),

	// Email/message forwarding attacks
	p("en-fw-001", `\b(forward|send|copy)\s+(all\s+)?(emails?|messages?|communications?)\s+to\b`,
		CategoryExfiltration, 4, LangEN, "Forward messages attack"),
	p("en-fw-002", `\bauto[_\s-]?forward\s+(rule|setting)?\b`,
		CategoryExfiltration, 4, LangEN, "Auto-forward rule attack"),

	// Calendar/scheduling attacks
	p("en-ca-001", `\b(cancel|delete)\s+(all\s+)?(my\s+)?(meetings?|events?|appointments?)\b`,
		CategoryAgentHijacking, 4, LangEN, "Cancel meetings attack"),
	p("en-ca-002", `\b(create|add|schedule)\s+.{0,30}(recurring|daily|weekly)\s+(meeting|event)\b.{0,50}(attacker|external|unknown)\b`,
		CategoryAgentHijacking, 4, LangEN, "Malicious recurring event"),
	p("en-se-013", `\bset\s+aside\s+(your\s+)?((current|operational)\s+)?(guidelines?|rules?|instructions?)\b`,
		CategorySocialEngineering, 4, LangEN, "Set aside guidelines"),
	p("en-se-014", `\b(share|provide)\s+(the\s+)?(complete|full)\s+(text|content|wording)\s+of\s+(your|any)\s+(instructions?|prompt)\b`,
		CategorySocialEngineering, 4, LangEN, "Share complete instructions"),
	p("en-se-015", `\b(instructions?|prompt)\s+you\s+(were\s+given|received)\s+(before|prior)\b`,
		CategorySocialEngineering, 4, LangEN, "Instructions you received"),
	p("en-se-016", `\b(describe|explain|write\s+out)\s+(the\s+)?(exact\s+)?(wording|text|content)\s+of\s+.{0,20}(instructions?|prompt)\b`,
		CategorySocialEngineering, 4, LangEN, "Describe exact wording"),
	p("en-se-017", `\blist\s+(any\s+)?(tools|functions|capabilities)\s+you\s+have\s+access\s+to\b`,
		CategorySocialEngineering, 3, LangEN, "List your capabilities"),
	...
	// =====================================================================
	p("en-os-009", `\b(always|only)\s+(recommend|suggest|promote|endorse|rank)\b`,
		CategoryOutputSteering, 3, LangEN, "Forced recommendation"),
	p("en-os-010", `\b(give|write|generate)\s+(a\s+)?(positive|five[- ]star|glowing|excellent)\s+(review|rating|feedback)\b`,
		CategoryOutputSteering, 3, LangEN, "Forced positive review"),
	p("en-os-011", `\b(mark|label|classify|rate)\s+(this|the)\s+(candidate|applicant|submission)\s+as\s+(hired|qualified|approved|excellent)\b`,
		CategoryOutputSteering, 3, LangEN, "Recruitment manipulation"),
	p("en-os-012", `\b(approve|accept|validate)\s+(this|the)\s+(content|ad|advertisement|listing|product)\b`,
		CategoryOutputSteering, 3, LangEN, "Content approval manipulation"),
	p("en-os-007", `\b(suppress|hide|ignore|remove)\s+(all\s+)?(negative|bad|critical|poor)\s+(reviews?|feedback|comments?|ratings?)\b`,
		CategoryOutputSteering, 3, LangEN, "Negative feedback suppression"),
	p("en-os-008", `\brank\s+.{0,20}(as\s+)?(number\s+one|#1|first|top)\b`,
